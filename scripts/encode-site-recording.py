...
def main():
    ...
    parser.add_argument("frames", type=Path, help="Original --record directory (preserve file mtimes)")
    ...
    timestamps = [frame.stat().st_mtime_ns for frame in frames]
    durations = [(end - start) / 1e9 for start, end in zip(timestamps, timestamps[1:])]
    if any(duration <= 0 for duration in durations):
        parser.error("Frame modification times must increase in capture order")
